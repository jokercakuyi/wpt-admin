  ...
  <div class="analysis">
    <a-row style="margin-top: 0" :gutter="[24, 24]">
      <a-col :sm="24" :md="12" :xl="8">
        <chart-card :loading="loading" :title="$t('totalSales')" :total="`￥${data.saleTotal}`">
...
<!--            <trend :term="$t('dod')" :target="100" :value="89" :scale="0" />-->
          ...
          <div slot="footer">{{$ta('daily|sales', 'p')}}<span> ￥{{ data.daySaleTotal }}</span></div>
        </chart-card>
      </a-col>
      <a-col :sm="24" :md="12" :xl="8">
        <chart-card :loading="loading" :title="$t('payments')" :total="data.payCount">
          <a-tooltip :title="$t('introduce')" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
...
<!--      </a-col>-->
      <a-col :sm="24" :md="12" :xl="8">
        <chart-card :loading="loading" :title="$t('company')">
          <a-avatar class="avatar" :size="96" shape="circle" :src="user.avatar" style="position: relative;transform: translateX(-50%);left: 50%"/>
          <!--          <a-tooltip :title="$t('introduce')" slot="action">-->
<!--            <a-icon type="info-circle-o" />-->
<!--          </a-tooltip>-->
<!--          <div>-->
...
import {getHomeInfo} from '@/services/culture'
...
export default {
  ...
  components: { Bar, MiniArea, ChartCard}
}
</script>
